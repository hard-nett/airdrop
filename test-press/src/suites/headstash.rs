...
use ict_rs::chain::terp::{
    Circuit, Contract, DeployedContract, TerpVmSuite, ZkSuiteError,
};
...
use cw_headstash::msg::ExecuteMsg as HeadstashExecuteMsg;
use cw_headstash_manifold::interface::CwHeadstashManifold;
use cw_headstash_manifold::msg::InstantiateMsg as ManifoldInstantiateMsg;
use cw_orch::prelude::*;

use crate::suite::{
    HeadstashBitwiseInstance, HeadstashIpfsInstance, HeadstashLaunchpadInstance,
    HeadstashSinsemillaTree, TestPressSuite,
};

use crate::suites::no_rick::NoRickDeployData;

use ict_rs::prelude::IctError;
...
impl HeadstashSuite {
    pub async fn store_circuit_binary(
        ...
        let out = ict_chain
            .chain_exec(&[
                "terpd", "tx", "wasm", "store-circuit", remote_path,
                "--from", signer_key,
...
impl Circuit for HeadstashZkCircuit {
    ...
    fn prove(
        ...
        let fvk = if bool::from(spk.is_some()) {
            ...
            return Err(ZkSuiteError::Circuit("SpendingKey derivation failed".into()));
        };
    ...
    fn verify(
        ...
        let cosmwasm_vk = zk_cosmwasm::VerifyingKey::new_with_params(
            vk.params.clone(),
            vk.vk.clone(),
            1,
        );
...
#[async_trait]
impl TerpVmSuite for HeadstashSuite {
    type Circuit = HeadstashZkCircuit;
    ...
    async fn deploy(
        ...
        let out = chain
            .chain_exec(&[
                "terpd", "tx", "wasm", "store-circuit", remote,
                "--from", deployer_key,
                "--gas", "auto", "--gas-adjustment", "1.5",
                "--output", "json", "-y",
            ])
