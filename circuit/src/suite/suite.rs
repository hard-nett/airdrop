...
pub trait CircuitKeysGenerator: HeadstashBitwiseInstance {
    ...
    fn gen_headstash_circuit_keys(
        ...
        println!("[3/4] Writing keys to {:?}...", keys_dir);

        // Write verifying key (params + vk + cs + footer)
        ...
        // Serialize the constraint system (v2 format)
        let mut cs = halo2_proofs::plonk::ConstraintSystem::<pallas::Base>::default();
        let _ = <crate::circuit::Circuit as halo2_proofs::plonk::Circuit<pallas::Base>>::configure(&mut cs);
        cs.write(&mut vk_file)?;

        // Append 32-byte v2 footer
        use std::io::Write;
        let footer = vk.generate_footer();
        vk_file.write_all(&footer.to_bytes())?;
        println!("  Written VK+CS+footer: {:?}", vk_path);

        println!("[4/4] Writing proving key...");
