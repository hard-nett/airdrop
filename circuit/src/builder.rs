...
use crate::{
    keys::{FullViewingKey, OutgoingViewingKey, SpendAuthorizingKey, SpendingKey},
...
use crate::{action::Action, circuit::Circuit};
...
impl SpendInfo {
    ...
    fn dummy(rng: &mut impl Rng) -> Self {
        let (sk, fvk, esk, note) = Note::dummy(rng, None);
    ...
    fn build(
        ...
        let nf_old = self.note.nullifier();
...
impl OutputInfo {
    ...
    ///
    /// [orcharddummynotes]: https://zips.z.cash/protocol/nu5.pdf#orcharddummynotes
    pub fn dummy(rng: &mut impl Rng) -> Self {
        let fvk: FullViewingKey = (&SpendingKey::random(rng)).into();
        // let recipient = fvk.address_at(0u32, Scope::External);
        let recipient = RecpAddr::new([0; 32]);
        let nd = NoteDenom::new_for_proof("denom");

        Self::new(None, nd, NoteValue::zero(), recipient, [0u8; 512])
    }
    ...
    fn build(
        ...
        //     out_ciphertext: encryptor.encrypt_outgoing_plaintext(v, &cmx, &mut rng),
        // };
...
impl ActionInfo {
    ...
    fn build(self, mut rng: impl Rng) -> (Action<SigningMetadata>, Circuit) {
        ...
        // let cv_net = ValueCommitment::derive(self.value_sum(), self.rcv.clone());

        let (nf_old, alpha) = self.spend.build(&mut rng);
        let (note, cmx) = self.output.build(nf_old, v, self.spend.note.nd(), &mut rng);
        ...
        (
            Action::from_parts(
                ...
                SigningMetadata {
                    ...
                    parts: SigningParts { alpha },
                },
            ),
            Circuit::from_action_context_unchecked(self.spend, note),
        )
    }
