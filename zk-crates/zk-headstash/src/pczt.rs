...
use crate::{
    ...
    tree::MerklePath,
    value::{NoteDenom, NoteValue, ValueCommitTrapdoor, ValueCommitment, ValueSum},
...
pub struct Spend {
    ...
    pub(crate) nd: Option<NoteDenom>,
    /// The note fixed-denomination index
    pub(crate) fdi: Option<u64>,
    pub(crate) esk: Option<EligibleSk>,
...
pub struct Output {
    ...
    /// The note denom of the output.
    pub(crate) nd: Option<NoteDenom>,

    /// The fdi for the  output.
    pub(crate) fdi: Option<u64>,
...
mod tests {
    ...
    use crate::{
        ...
        keys::{EligibleSk, FullViewingKey, Scope, SpendAuthorizingKey, SpendingKey},
    ...
    fn shielded_bundle() {
        ...
        let e_sk_secp = SecretKey::from_byte_array(e_sk_bytes).expect("valid secret key");
        ...
        let recp = RecpAddr::try_from(
            mock_deps
                .api
                .addr_canonicalize(&mock_deps.api.addr_make(&"dang").to_string())
                .unwrap(),
        )
        .unwrap();
