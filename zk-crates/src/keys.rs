...
use crate::spec::{elig_sk_to_base, prf_nf, prf_pallas_m, to_base};
...
impl NoteMessage {
    pub fn derive(v: NoteValue, nd: NoteDenom, fdi: u64, e_sk: EligibleSk) -> Self {
...
/// - e_sk: the product of the 3*88-bit limbs,on the pallas curve
...
impl NullifierDerivingKey {
    ...
    /// Derive nullifier key from e_sk & rho using HKDF.
    ///
    ...
    pub fn derive_from(e_sk: EligibleSk, rho: Rho) -> Self {
        // Convert secp256k1 secret key to pallas::Base representation
        let e_sk_pallas = crate::spec::elig_sk_to_base(&e_sk);
